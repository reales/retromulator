...
namespace synthLib
{
	...
	void ResamplerInOut::setChannelCounts(const uint32_t _channelCountIn, const uint32_t _channelCountOut)
	{
		if(m_channelCountIn == _channelCountIn && m_channelCountOut == _channelCountOut)
			return;

		m_channelCountIn  = _channelCountIn;
		m_channelCountOut = _channelCountOut;

		m_scaledInput = AudioBuffer(_channelCountIn);
		m_input       = AudioBuffer(_channelCountIn);
		m_scaledInputSize = 0;
	...
	void ResamplerInOut::process(const TAudioInputs& _inputs, TAudioOutputs& _outputs, const TMidiVec& _midiIn, TMidiVec& _midiOut, const uint32_t _numSamples, const TProcessFunc& _processFunc)
	{
		...
		[[maybe_unused]] const auto outputSize = m_out->process(_outputs, m_channelCountOut, _numSamples, false, feedOutput);
